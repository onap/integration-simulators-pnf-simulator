...
## Fetching certificates from Certman
### Description
...
## Fetching certificates from Certservice
### Description
...
uses certificates generated by Certservice container. 

##### VES collector local deployment prerequisites

By default the image of VES from Nexus is supporting only HTTP communication. Local image with enabled HTTPS must be 
...
``sanitycheck/events/eventToVes.json`` file which is request for sending event to VES must have correct ``vesServerURL`` 
field before sending event. For local VES it should have value: 
